...
#and location afganistan
df_afghanistan = sqldf('''
select * from df_covid_all where location = "Afghanistan" and total_cases IS NOT NULL and total_deaths IS NOT NULL
''')

count_data = df_afghanistan.shape[0]
#load data after transformation to postgresql
TABLE_NAME = 'data_covid_afghanistan'
df_afghanistan.to_sql(name=TABLE_NAME, con=engine,index=False,if_exists='replace')
